...
/datum/species/beefman
	...
	bodytemp_normal = (BODYTEMP_NORMAL - 50) //Freezing temperatures is what they like
...
/datum/species/beefman/spec_life(mob/living/carbon/human/H)	// This is your life ticker.
	..()
	// 		** BLEED YOUR JUICES **         // BODYTEMP_NORMAL ~ 260
...
/datum/species/beefman/spec_attacked_by(obj/item/I, mob/living/user, obj/item/bodypart/affecting, intent, mob/living/carbon/human/H)
	...
	if (H.stat < DEAD && !affecting && intent == INTENT_DISARM && istype(I, /obj/item/food/meat/slab))// /obj/item/bodypart))
		var/target_zone = user.zone_selected
		var/list/allowedList = list ( BODY_ZONE_L_ARM, BODY_ZONE_R_ARM, BODY_ZONE_L_LEG, BODY_ZONE_R_LEG )
...
/mob/living/carbon
	...
	var/obj/item/bodypart/r_leg/part_default_r_leg = /obj/item/bodypart/r_leg
		...
		// MEAT-TO-LIMB
		// 1) Save Meat's type
		// 2) Get all original Reagent TYPES from "food_reagents" on the meat itself - these reagents (TYPEPATHS) have the starter values. Save those values.
		...
		// 3) Percent = Compare the STARTER VALUES in food_reagents against the CURRENT VALUES in thisMeat.reagents.reagent_list/
		// 4) Inject ALL OTHER CHEMS into bloodstream

		// LIMB-TO-MEAT
		// 1) Create new meat
		// 2) Sort through all reagent datums in newMeat.food_reagents and adjust each version in newMeat.reagents.reagent_list/(REAGENT)/.volume
...
/obj/item/bodypart/proc/give_meat(mob/living/carbon/human/H, obj/item/food/meat/slab/inMeatObj)
	...
	// Get Original Amount
	var/datum/component/edible/le_food = GetComponent(/datum/component/edible)
	var/amountOriginal
	...
	if (percentDamage >= 0.9)
		...
		to_chat(owner, "<span class='alert'>It's riddled with [le_food.bitecount > 0 ? "bite marks":"gaping holes"].</span>")
	else if (percentDamage > 0)
		to_chat(owner, "<span class='alert'>It looks a little [le_food.bitecount > 0 ? "eaten away":"torn up"], but it'll do.</span>")
