...
/obj/item/melee/twistedtea
	...
	var/attack_verb_continuous_b = list("taps", "hits", "pokes")
	var/attack_verb_simple_b = list("tap", "hit", "poke")
...
/obj/item/melee/twistedtea/proc/refill()
	if(!chem)
		return
	create_reagents(tanksize)
	reagents.add_reagent(chem, tanksize)
...
/obj/item/melee/twistedtea/attack(mob/living/target, mob/living/user)
	...
	if(!unlimiteduse)
		broken = TRUE
		name = "Broken Twisted Tea"
		icon = 'icons/obj/janitor.dmi'
		icon_state = "sodawater"
		attack_verb_continuous = attack_verb_continuous_b
		attack_verb_simple = attack_verb_simple_b
	else
		refill()
